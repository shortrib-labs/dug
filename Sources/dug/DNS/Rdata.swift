...
enum Rdata: Equatable {
    ...
    case soa(mname: String, rname: String, serial: UInt32,
    ...
    var shortDescription: String {
        switch self {
        case .a(let addr): return addr
        case .aaaa(let addr): return addr
        case .cname(let name): return name
        case .ns(let name): return name
        case .ptr(let name): return name
        case .mx(let pref, let exchange): return "\(pref) \(exchange)"
        case .soa(let mn, let rn, let ser, let ref, let ret, let exp, let min):
            return "\(mn) \(rn) \(ser) \(ref) \(ret) \(exp) \(min)"
        case .srv(let pri, let w, let port, let target):
